<?php
/**
 * Pengabdian (Community Service) Archive Page - Pastel Theme
 * Display community service documents with PDF download
 */
...
// Build query
$conditions = "is_active = TRUE AND category = 'pengabdian'";
$params = [];
...
// Get stats
$totalDownloads = db()->fetch("SELECT COALESCE(SUM(download_count), 0) as total FROM documents WHERE is_active = TRUE AND category = 'pengabdian'");
?>

<!-- Page Header -->
<section class="py-20 bg-gradient-to-b from-[#171c28] to-[#1e2433] relative overflow-hidden">
    <div class="absolute inset-0 opacity-10">
        <div class="absolute w-96 h-96 -top-48 -right-48 bg-[#c3b1e1] rounded-full blur-3xl"></div>
        <div class="absolute w-96 h-96 -bottom-48 -left-48 bg-[#e8b4bc] rounded-full blur-3xl"></div>
    </div>
    ...
    <div class="container mx-auto px-4 relative z-10">
        ...
        <nav class="mb-8" data-aos="fade-down">
            <ol class="flex items-center space-x-2 text-sm">
                ...
                <li><span class="text-gray-600">/</span></li>
                <li><span class="text-[#c3b1e1]">Pengabdian</span></li>
            </ol>
        </nav>
        
        <!-- Page Title -->
        <div class="max-w-3xl">
            <span class="inline-block px-4 py-2 bg-[#c3b1e1]/10 border border-[#c3b1e1]/30 rounded-full text-[#c3b1e1] text-sm font-semibold mb-4" data-aos="fade-up">
            ...
            <p class="text-gray-400 text-lg" data-aos="fade-up" data-aos-delay="200">
                Koleksi dokumen pengabdian kepada masyarakat
            </p>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 mt-8" data-aos="fade-up" data-aos-delay="300">
            <div class="bg-[#2a3142] rounded-xl p-4 border border-[#3a4255]">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 bg-[#c3b1e1]/10 rounded-lg flex items-center justify-center">
                        <i class="fas fa-file-pdf text-[#c3b1e1]"></i>
                    </div>
                    <div>
            ...
            <div class="bg-[#2a3142] rounded-xl p-4 border border-[#3a4255] hidden md:block">
                <div class="flex items-center space-x-3">
                    <div class="w-10 h-10 bg-[#e8b4bc]/10 rounded-lg flex items-center justify-center">
                        <i class="fas fa-hands-helping text-[#e8b4bc]"></i>
                    </div>
                    <div>
                        <p class="text-2xl font-bold text-white">Service</p>
        ...
        <div class="mt-8 max-w-xl" data-aos="fade-up" data-aos-delay="400">
            <form action="<?= baseUrl() ?>" method="GET" class="relative">
                ...
                <input type="text" name="q" value="<?= htmlspecialchars($search) ?>" 
                       placeholder="Cari dokumen pengabdian..."
                       class="w-full px-5 py-4 pl-12 bg-[#2a3142] border border-[#3a4255] rounded-xl text-white placeholder-gray-500 focus:border-[#c3b1e1] focus:ring-1 focus:ring-[#c3b1e1] transition-all">
                <i class="fas fa-search absolute left-4 top-1/2 -translate-y-1/2 text-gray-500"></i>
                <button type="submit" class="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-2 bg-[#c3b1e1] text-gray-800 rounded-lg hover:bg-[#e8b4bc] transition-colors">
                    Cari
                </button>
            </form>
        </div>
    ...
    <div class="container mx-auto px-4">
        ...
        <div class="mb-8 flex items-center justify-between">
            <p class="text-gray-400">
                Menampilkan hasil untuk: <span class="text-[#c3b1e1] font-semibold">"<?= htmlspecialchars($search) ?>"</span>
                <span class="text-gray-500">(<?= $totalItems ?> hasil)</span>
            </p>
            <a href="<?= baseUrl('?page=pengabdian') ?>" class="text-[#c3b1e1] hover:text-[#e8b4bc] text-sm">
        ...
        <div class="space-y-4">
            ...
            <div class="cyber-card rounded-xl p-6 hover:border-[#c3b1e1]/50 transition-all" data-aos="fade-up" data-aos-delay="<?= $index * 50 ?>">
                <div class="flex flex-col md:flex-row md:items-center gap-6">
                    <!-- Document Icon -->
                    <div class="flex-shrink-0">
                        <div class="w-16 h-20 bg-gradient-to-br from-[#c3b1e1] to-[#e8b4bc] rounded-lg flex items-center justify-center shadow-lg shadow-[#c3b1e1]/20">
                    ...
                    <!-- Document Info -->
                    <div class="flex-1">
                        <h3 class="font-display text-lg font-bold text-white mb-2 hover:text-[#c3b1e1] transition-colors">
                    ...
                    <div class="flex-shrink-0">
                        <a href="<?= baseUrl('?page=download&id=' . $doc['id']) ?>" 
                           class="cyber-btn inline-flex items-center px-6 py-3 bg-gradient-to-r from-[#c3b1e1] to-[#e8b4bc] text-gray-800 font-semibold rounded-xl hover:opacity-90 transition-all shadow-lg shadow-[#c3b1e1]/20">
        ...
        <!-- Pagination -->
        <?php
        $baseUrlPagination = baseUrl('?page=pengabdian' . (!empty($search) ? '&q=' . urlencode($search) : ''));
        ...
        <div class="text-center py-20">
            <div class="w-24 h-24 bg-[#2a3142] rounded-full flex items-center justify-center mx-auto mb-6">
                <i class="fas fa-hands-helping text-gray-600 text-4xl"></i>
            </div>
            <h3 class="text-xl font-semibold text-gray-400 mb-2">
                <?= !empty($search) ? 'Tidak ada hasil ditemukan' : 'Belum ada dokumen pengabdian' ?>
            </h3>
            <p class="text-gray-500">
                <?= !empty($search) ? 'Coba gunakan kata kunci yang berbeda' : 'Dokumen pengabdian akan segera ditampilkan' ?>
            </p>
        </div>
        <?php endif; ?>
    </div>
